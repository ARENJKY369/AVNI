...
import { inspectPdf, lastBlobBytes } from './pdf-inspect.mjs';
...
// export row: both downloads are visible buttons, and the geometry one is
// disabled while the scene has no CRS
const geoItem = await page.evaluate(() => {
  const b = [...document.querySelectorAll('button')].find((x) => /Download GeoJSON/i.test(x.textContent));
  return b ? { disabled: b.disabled, title: b.title, text: b.textContent.replace(/\s+/g, ' ').trim() } : null;
});
check(
  'GeoJSON download disabled when unlocated',
  !!geoItem && geoItem.disabled && /withheld/.test(geoItem.title),
...
await page.evaluate(() => [...document.querySelectorAll('button')].find((b) => /evidence bundle/i.test(b.textContent))?.click());
...
// ---------------------------------------------------------------- PDF report
// The report has to be a real document: the sections, the payload's own
// numbers, a map snapshot of this AOI and no coordinates that the console
// itself is withholding.
await page.evaluate(() => [...document.querySelectorAll('button')].find((b) => /Download report \(PDF\)/i.test(b.textContent))?.click());
await new Promise((r) => setTimeout(r, 9000));
const pdfBlob = await lastBlobBytes(page);
check(
  'Download report (PDF) writes a PDF',
  !!pdfBlob && pdfBlob.type === 'application/pdf' && pdfBlob.size > 20000,
  pdfBlob ? `${pdfBlob.type} · ${(pdfBlob.size / 1024).toFixed(0)} kB` : 'no blob captured'
);
let report = null;
if (pdfBlob) {
  report = await inspectPdf(Buffer.from(pdfBlob.base64, 'base64'));
  const wanted = [
  ...
  const missing = wanted.filter((w) => !report.text.includes(w));
  check(
    'the report carries its sections, the answer and the AOI name',
    missing.length === 0,
    missing.length ? `missing ${JSON.stringify(missing)}` : `${report.pages} pages · ${report.text.length} chars of text`
  );
  check(
    'the report embeds a map snapshot',
    report.images >= 1 && report.jpeg,
    `${report.images} image object(s) · jpeg=${report.jpeg}`
  );
  check(
    'the report prints the scene it was composed from, not the bundled one',
    report.text.includes('S2_L2A_2025-08-14_tile43REP.jp2') === false &&
      /avni-verify-whitefield\.tif/.test(report.text),
    (report.text.match(/[^\n]*\.tif[^\n]*/g) || []).join(' | ') || 'no scene filename found'
  );
  const geoFilename = (report.text.match(/avni_answer_[a-z0-9-]+\.geojson/) || [])[0];
  check(
    'the footer cross-references the GeoJSON export filename',
    !!geoFilename && report.text.includes(`Geometry cross-reference: ${geoFilename}`),
    geoFilename || 'no cross-reference found'
  );
  const coordsInReport = (report.text.match(/\d{1,2}\.\d{4}°[NS]/g) || []).length;
  check(
    'the georeferenced report does print coordinates',
    coordsInReport >= 1,
    `${coordsInReport} coordinate value(s) on the page`
  );
}
